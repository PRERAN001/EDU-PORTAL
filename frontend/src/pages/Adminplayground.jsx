...
const Adminplayground = () => {
  ...
  const changeresoursename = useRef(null);
    ...
    try {
      const [resV, resR] = await Promise.all([
        ...
        fetch(
          ...
          }/user/list/resources/${department}`
        ),
      ]);
  ...
  const { admindept,currentadmin } = useContext(SubjectContext);
  const departments = [
    "Edge_computing",
    "Genome_Sequence",
    "Geometry_Analysis",
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_backend_url}/fileaccess/addadminforfiles`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ filename, fileuploadedby: currentadmin.email }),
        }
      );
      if (res.status === 201) {
        return true;
  ...
  const handleSubmit = async (e, type) => {
    ...
    const videoName = changevideonaem.current.value.trim();
    const resourceName = changeresoursename.current.value.trim();
    const originalFile = type === "video" ? video : resource;
    const ext = type === "video" ? ".mp4" : ".pdf";
    const customName = type === "video" ? videoName : resourceName;
    const finalName = customName !== "" ? customName : originalFile.name;
    ...
    formData.append(type, renamedFile);
  ...
  const handleDelete = async(filename, type) => {
    ...
    const hasAccess = await checkFileAccess(filename);
    if (!hasAccess) {
    ...
    const deleteFileUrl = `${
      import.meta.env.VITE_backend_url
    }/user/delete/${department}/${encoded}`;
    const deleteResourceUrl = `${
      import.meta.env.VITE_backend_url
    }/user/delete/${department}/resources/${encoded}`;
